...
### 1-3. 발언자 메타데이터 고도화 ✅ 완료

- [x] **정당 룩업 테이블** (`data/speakers.json`): 국민의힘·더불어민주당·조국혁신당 정적 매핑 (2026-06-24)
- [x] **ETL 단계 정당 자동 분류** (`chunker_v2.py`): 장관·차관 등 직함 → 정부측, 의원 → speakers.json 룩업 (2026-06-24)
- [x] **embed_text 정당 포함**: 임베딩 텍스트에 정당 정보 주입 → "민주당 입장", "야당 발언" 의미 검색 강화 (2026-06-24)
- [x] **전체 청크 메타 UPSERT**: 13,255 청크 party/position_type 반영 완료, 미확인 의원 0명 (2026-06-24)

### 1-4. 임베딩 모델 업그레이드 (나중에)

- [ ] 현재 `multilingual-e5-small` → `multilingual-e5-large` 또는 `bge-m3`로 교체
- [ ] 임베딩 차원 확장 후 recall@3 변화 비교 실험

> 1-3 완료 + 파이프라인 안정화 후 진행.
...
- [x] **스트리밍 응답**: 답변이 실시간으로 타이핑되듯 출력 (2026-06-21)
...
### 3-4. LLM 컨텍스트 품질 고도화 ✅ 완료

- [x] **prev_context / next_context**: 청크 앞뒤 발언 100자 → ETL 저장 → LLM 컨텍스트 전달 (2026-06-24)
- [x] **발언자 정당 LLM 전달**: `_build_numbered_context()` 수정 → `이재정 위원 (더불어민주당)` 형식으로 LLM에 전달 (2026-06-24)
- [x] **max_tokens 이중관리 해소**: generate.py · chat.py 스트리밍 경로 통일, 512 → **1024** (2026-06-24)
- [x] **`_est_tokens()` 한국어 보정**: `// 4` → `// 2`, 로그 추정치 정확도 향상 (2026-06-24)
- [ ] **프롬프트 정당 해석 가이드**: 시스템 프롬프트에 여당/야당/정부측 해석 맥락 명시
- [ ] **여야 비교 쿼리 자동 감지**: "여야", "민주당", "국민의힘" 키워드 → balance_speakers 자동 ON
...
| 순위 | 항목 | 임팩트 | 난이도 | 상태 |
|------|------|--------|--------|------|
| ⭐⭐⭐ | 프롬프트 정당 해석 가이드 | 여야 비교 답변 품질 | 낮 | 미완 |
| ⭐⭐⭐ | 여야 비교 쿼리 자동 감지 | balance_speakers 자동화 | 낮 | 미완 |
| ⭐⭐⭐ | Day 14 재현성 패키징 | 포트폴리오 | 중 | 미완 |
| ⭐⭐⭐ | Day 15 데모·발표 | 포트폴리오 | 중 | 미완 |
| ⭐⭐ | 데이터 확장 (전체 위원회) | 커버리지 | 중 | 미완 |
| ⭐⭐ | 구조화 출력 (JSON) | 재현성 | 중 | 미완 |
| ⭐ | e5-large / bge-m3 교체 | 검색 품질 | 높 | 미완 |
| ✅ | 발언자 정당 메타 고도화 | 여야 구분 검색·답변 | — | 완료 |
| ✅ | LLM 컨텍스트 버그 수정 | prev/next·정당 전달 | — | 완료 |
| ✅ | max_tokens 1024 통일 | 답변 완성도 | — | 완료 |
